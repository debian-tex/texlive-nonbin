...
echo "=== Test stable upgrade: dist-upgrade pdfjam from stable to test ==="
echo ""

echo "=== START INSTALL STABLE VERSION"
apt-get install $aptargs pdfjam
find /etc/texmf | sort > /pool/stable-test-5-files-pre
...
find /etc/texmf | sort > /pool/stable-test-5-files-post
